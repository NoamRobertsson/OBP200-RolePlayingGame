...
class Program
{
    ...
    static bool DoBattle(bool isBoss)
    {
        ...
        while (enemyHp > 0 && !player.IsDead())
        {
            ...
            if (cmd == "A")
            {
                int damage = player.DealDamage(enemyDef, Rng);
                enemyHp -= damage;
    ...
    static int CalculateEnemyDamage(int enemyAtk)
    {
        ...
        int dmg = Math.Max(1, enemyAtk - (player.Def / 2)) + roll;
    ...
    static bool DoShop()
    {
        ...
        while (true)
        {
            ...
            if (val == "1")
            {
                player.TryBuy(new BuyPotion());
            }
            else if (val == "2")
            {
                player.TryBuy(new BuyWeapon());
            }
            else if (val == "3")
            {
                player.TryBuy(new BuyArmor());
            }
            else if (val == "4")
            {
                player.SellMinorGems();
            }
            else if (val == "5")
            {
